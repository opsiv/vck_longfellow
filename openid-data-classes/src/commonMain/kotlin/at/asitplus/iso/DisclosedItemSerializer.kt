...
open class DisclosedItemSerializer(private val namespace: String, private val elementIdentifier: String) :
    KSerializer<DisclosedItem> {

    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("DisclosedItem") {
        element(DisclosedItem.PROP_ELEMENT_ID, String.serializer().descriptor)
        element(DisclosedItem.PROP_ELEMENT_VALUE, String.serializer().descriptor)
    }

    override fun serialize(encoder: Encoder, value: DisclosedItem) {
    ...
    private fun CompositeEncoder.encodeAnything(value: DisclosedItem, index: Int) {
        val elementValueSerializer = buildElementValueSerializer(namespace, value.elementValue, value.elementIdentifier)
        val descriptor = buildClassSerialDescriptor("DisclosedItem") {
            element(DisclosedItem.PROP_ELEMENT_ID, String.serializer().descriptor)
            element(DisclosedItem.PROP_ELEMENT_VALUE, elementValueSerializer.descriptor, value.elementValue.annotations())
        }
    ...
    override fun deserialize(decoder: Decoder): DisclosedItem {
        var digestId = 0U
        ...
        decoder.decodeStructure(descriptor) {
            while (true) {
                ...
                when (name) {
                    DisclosedItem.PROP_ELEMENT_ID -> if (elementIdentifier != decodeStringElement(descriptor, index))
                        throw IllegalArgumentException("Element identifier mismatch")

                    DisclosedItem.PROP_ELEMENT_VALUE -> elementValue = decodeAnything(index, elementIdentifier)
                }
                if (elementValue != null) break
            }
        }
